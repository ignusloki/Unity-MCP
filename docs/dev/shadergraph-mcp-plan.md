...
## Epic 7H: WorldSpaceDepthFade Node Gap

Status:

- Implemented in code on `custom/shadergraph-mcp`.
- Compile sanity check passed in the local Unity 6 validation project on 2026-06-20 with existing Unity/API warnings and 0 errors.
- Live single-op MCP validation passed in the already-open Unity editor on 2026-06-20.

Purpose:

- Close the WorldSpaceDepthFade trial blocker reported by `Unity-test/TestShadergraph/Assets/Unity-MCP-Test/Trials/WorldSpaceDepthFade/TRIAL_REPORT.md`.
- Add the missing behavior-relevant nodes needed to reconstruct the reference depth-fade path without fake topology or raw `.shadergraph` edits.

Implementation plan:

- Slice 7H.1: add `camera` to `assets-shadergraph-add-node` as `UnityEditor.ShaderGraph.CameraNode`; readback exposes output slots such as `Position`, `Direction`, `Orthographic`, `Near Plane`, `Far Plane`, `Z Buffer Sign`, `Width`, and `Height`. Implemented.
...
- `dotnet build Assembly-CSharp.csproj -v minimal` must pass with 0 errors.
- Focused editor tests must cover `Validation_AddNode_WorldSpaceDepthFade.shadergraph`, `Validation_UpdateNodeSettings_WorldSpaceDepthFade.shadergraph`, and `Validation_WorldSpaceDepthFadePath.shadergraph`.
- Live validation should use single-op tools until the known batch rollback issue is handled.
...
- **Epic 8B v1 complete on 2026-06-19.** All seven planned slices (alias foundation + batch tool, filtered query-structure, response modes, idempotent connect, blackboard category cleanup, clean URP template, Vector3 → UV auto-narrow) have landed on `custom/shadergraph-mcp` (commits `89b24b48` through `25089bd1`) and validated against the live Unity 6000.4.1f1 editor in `Unity-test/TestShadergraph` after switching that project to the local package source. v2 follow-up work (shared in-memory `GraphData`, single-op tool migration to `ShaderGraphResponseMode`, idempotent semantics on the remaining mutation tools, clean Lit/HDRP templates) is tracked in `docs/dev/futureDebt.MD`.
- **Dissolve-4 trial gap fixes complete on 2026-06-20.** The Dissolve-4 trial report (`Unity-test/TestShadergraph/Assets/Unity-MCP-Test/Trials/Dissolve-4/TRIAL_REPORT.md`) found four follow-ups after the trial halved authoring round-trips. All four landed in this commit and were live-validated:
  - **D-1 — Batch alias selectors.** `ShaderGraphUpdateNodeSettingsInput.Node` (`ShaderGraphNodeRef`), `ShaderGraphPropertyUpdateInput.Property` (`ShaderGraphPropertyRef`), and `ShaderGraphAddPropertyNodeInput.Property` (`ShaderGraphPropertyRef`) let the batch refer to a previously-aliased node/property without round-tripping through the alias map. `ResolveNodeIdOrAlias` and `ResolvePropertyIdOrAlias` also strip a leading `@` so `"@noise"` and bare `"noise"` both resolve.
  ...
  - **D-2 — Retry-safe rollback attempt.** On `stopOnError=true` failure the batch attempts to restore the pre-batch disk snapshot, bump the file mtime, import the graph, reload open ShaderGraph windows, and verify the restored on-disk hash. This path remains provisional until the known rollback bug is fixed.
...
- **Dissolve-5 rollback hardening remains provisional and should not be treated as accepted.** The Dissolve-5 trial report (`Unity-test/TestShadergraph/Assets/Unity-MCP-Test/Trials/Dissolve-5/TRIAL_REPORT.md`) flagged a destructive rollback risk after delete/recreate/failed-batch flows. A hardening attempt exists on the branch, but the known rollback bug is intentionally deferred; use single-op mutation tools for live validation until the rollback snapshot/finalizer behavior is reworked and revalidated.
  ...
  - **E-2 — Snapshot hash verification attempt.** Every batch hashes the snapshot bytes at start and again after disk-safe rollback import/reload, but this should be treated as diagnostic-only until the rollback bug is fixed. If verification fails, the message reports `WARNING: rollback verification FAILED. Snapshot hash X but on-disk hash Y after restore.`
  - **E-3 — Disk-write finalizer.** Added a separate finalization path for template clones and rollback restores that imports/reloads from disk without `AssetDatabase.SaveAssets()`. Live MCP validation on `Codex_BatchRollback_DiskSnapshot.shadergraph` confirmed an overwrite-created clean graph remained byte-identical after a failed op[0] duplicate-property batch, with stale marker content not restored.
- **WorldSpaceDepthFade trial fix landed on 2026-06-20.** The trial report (`Unity-test/TestShadergraph/Assets/Unity-MCP-Test/Trials/WorldSpaceDepthFade/TRIAL_REPORT.md`) flagged three gaps after the previously-missing `camera` / `exponential` nodes started working. Gap 1 reproduced in a controlled live test (`InvalidOperationException: Shader Graph node '<id>' did not change.`). Gap 3 dissolved into Gap 1 once we fixed the root throw. Gap 2 is outside this repo and is tracked in `futureDebt.MD`. The single landed fix:
  - **WSDF-1 — No-op `updateNodeSettings` is a success, not a failure.** `UpdateSampleTexture2DNodeSettings` and the general `UpdateSerializedNodeSettings` dispatch both routed the `changedFields.Count == 0` case to `throw new InvalidOperationException`; a single no-op (e.g. `View Vector.Space=world` when the node is already at world space) aborted the whole batch via the rollback path. Both call sites now route to a new `BuildNodeSettingsNoOpResult` helper that returns `Operation="updateSettings"`, `Node=<current snapshot>`, `ChangedFields=[]`, and a new boolean `NoOp=true` on `ShaderGraphNodeMutationResultData`. The asset is NOT re-imported — `ValidateGraphMethod`, `SaveShaderGraphReflectionDocument`, and `FinalizeShaderGraphMutation` are all skipped on the no-op path. The batch tool's loop continues; the op shows up in the per-op result array as `Success=true` with an empty diff. Live-validated: a single-op `View Vector.Space=world` no-op returns `NoOp=True, ChangedFields=[]`; a 3-op batch where op[1] is the same no-op now completes all 3 ops with `Success=True` instead of rolling back.
  - **Gap 2 — HTTP MCP "Response data is null".** Outside this repo. The gap is in the GameDev-MCP-Server (the engine-agnostic standalone MCP server pinned in `McpServerManager.cs`), not in the Unity-MCP plugin. Reproduced locally — a long script-execute call returned `Response data is null` with no underlying message while a smaller variant of the same script returned the real exception. Tracked as v2 debt in `docs/dev/futureDebt.MD` so the server team can investigate response framing / size limits.
  - **Gap 3 — Wiring batch "failure with completed state".** Dissolves into WSDF-1. The trial's wiring batch failed at op[0] with the same "node did not change" error as Gap 1; the pre-batch fingerprint then reported the prior successful batch's state (the agent's first batch having already completed). With WSDF-1 the wiring batch never aborts at op[0] in the first place, so there is no fingerprint-versus-readback mismatch to misread.
...
- 2026-06-19 live-Unity validation against `Unity-test/TestShadergraph` (project switched to file: source of the local package). Per slice:
  - Slice 8B.1 + 8B.3 batch: alias map registered for `addNode` ops; later `connectEdge` resolved against the bag; consolidated `GraphSummary` reflected post-batch counts; `Selection` / `Structure` null in `Summary` mode. Bug found and fixed in the same session: the batch's `connectEdge` resolver wrote the legacy `*ObjectId` fields but did not clear `OutputSlot` / `InputSlot`, so `ConnectShaderGraphEdge` re-ran the resolver with `aliases=null` and threw. Fix landed in commit `d4aaccc5` (`ApplyConnectEdgeSlotRefs` now nulls the SlotRefs after a successful resolution).
  - Slice 8B.2 query-structure: `StatsOnly`, `PropertiesOnly`, `NodeTypeSubstrings` with `IncludeSlots=false` / `IncludeNodeSettings=false` / `IncludeEdges=false` / `IncludeTargets=false` returned the expected subsets; non-selected lists were null.
  - Slice 8B.3 response modes: `Diff` strips `GraphSummary`, `Selection` is exactly the single touched node, `Full` echoes the entire structure.
  - Slice 8B.4 idempotent connect: re-running the same `connectEdge` with `AllowExisting=true` returned `AlreadyExisted=true` + `ChangedFields=[edge.alreadyExists]` and skipped the import. Without the flag, the duplicate threw loudly. The batch `connectEdge` op honoured the same flag.
  - Slice 8B.5 category cleanup: `delete-category` on an empty category removed it; on a non-empty category without `ReassignPropertiesToDefault` threw "category 'X' is not empty (1 properties)"; with `ReassignPropertiesToDefault=true`, moved the child property to the default category before deletion. `prune-empty-categories` removed two empty categories; a second prune was a fast no-op (no re-import).
  - Slice 8B.6 clean URP template: `useCleanUrpTemplate=true` produced 6 block-stack nodes / 0 edges / 0 properties / 1 UniversalTarget. `ShaderResolved=true`, `HasErrors=false`. The default template control still produced 10 nodes / 2 properties / 2 targets.
  - Slice 8B.7 Vector3 → UV auto-narrow: without the flag, `ValidateEdgeCompatibility` threw `"Unsupported slot compatibility: 'Vector3MaterialSlot' -> 'UVMaterialSlot'"`. With `AutoNarrowVector3ToUV=true`, the result reported `AutoCreatedNodeObjectIds.Count=2` (Split + Combine), `AutoCreatedEdges.Count=3`, a populated final `Edge`, and the post-state contained Split + Combine. `ShaderResolved=true`, `HasErrors=false`.
- One known minor cosmetic issue (not blocking): on slice 8B.5 `delete-category` with `CategoryName=""`, the empty-string resolver early-returns with a generic *"Either categoryObjectId or categoryName must be provided"* before the dedicated "default category cannot be deleted" guard fires. The default category is still protected, just with a less specific error message.
